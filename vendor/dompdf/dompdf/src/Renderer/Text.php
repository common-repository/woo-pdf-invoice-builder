...
class Text extends AbstractRenderer
{
    ...
    function render(Frame $frame)
    {
        ...
        if (preg_match("/([\x{0590}-\x{06FF}\x{0700}-\x{085F}\x{FB00}-\x{FDFD}\x{FE70}-\x{FEFF}])/u", $text)||$wooForceOTL)
            $text=TextProcessor::GetInstance($this->_dompdf)->Process($text,$style->get_font_family_name());
